...
def kron_moments_partial_obs(obs : Observable, avg_key : str, num_moments : int, mem_avail : float, n_samples : int):
    keys, vals, keynames= obs.get_merged_key_data(avg_key)

    samples_found = len(vals[0])
    if(n_samples > samples_found):
        raise ValueError(str(n_samples) + " samples were requested but only " + str(samples_found) + " samples were found")

    print("Processing " + str(n_samples) + " out of " + str(samples_found) + " samples in observable " + obs.get_name())
    vals = vals[:,:n_samples]

    moms_obs = []

    for k in range(1, num_moments+1):
        name = obs.props.loc[0,'name'] + "_moms(" + avg_key + "_" + str(n_samples) +")" + "_k" + str(k)
...
def kron_moments_partial(avg_key, num_moments, mem_avail, n_samples, parallelizer, disk_format):
    ...
    for i in range(parallelizer.numfiles):
        moms_obs = kron_moments_partial_obs(parallelizer.get_obs(i), avg_key, num_moments, mem_avail, n_samples)
        for obs in moms_obs:
